...
const CompanionComponent = ({ companionId, subject, topic, name, userName, userImage, style, voice }: CompanionComponentProps) => {
    ...
    return (
        <section className="flex flex-col h-screen max-h-[90vh] overflow-hidden">
            {/* Top section - Fixed height */}
            <section className="flex gap-8 max-sm:flex-col mb-4 flex-shrink-0">
            ...
            <section className="transcript flex-1 min-h-0 relative">
                <div className="transcript-message no-scrollbar h-full overflow-y-auto pb-8">
                    {messages.length === 0 ? (
                        <div className="flex items-center justify-center h-full text-gray-500">
                            <p>Transcript will appear here when you start the session...</p>
                        </div>
                    ) : (
                ...
                <div className="transcript-fade absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-white to-transparent pointer-events-none" />
            </section>
        </section>
    )
}
